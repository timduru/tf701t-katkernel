...
static int default_mean_time = 31;//0x1f
...
// als calibration
#define CAL_ALS_PATH "/persist/lightsensor/ALS_Config.ini"
...
static int al3320a_set_als_mean_time(struct i2c_client *client, int mean_time){
    int err = 0;
//+++ don't restrict mean_time range , because current mean_time include some special bits.
                     ...
                     char *buf)
{
    ...
    printk("al3320a : al3320a_als_on_by_default_config(client) = %d\n",ret);
    ret = al3320a_get_sys_config_als_enable(client);
    printk("al3320a : al3320a_get_sys_config_als_enable(client) = %d\n",ret);
    if(ret<0){
        ret = 0;
    }else{
        ret = 1;
