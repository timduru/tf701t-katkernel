...
/*****************************************************************************/
/* for probe time reset */
/*****************************************************************************/
/* registers */
#define ACCEL_INT_REL			0x1A
#define ACCEL_CTRL_REG1			0x1B
#define ACCEL_SRST_CTRL_REG		0x1D
/* register values */
#define ACCEL_PC1_OFF			0x7F
#define ACCEL_PC1_ON			(1 << 7)
#define ACCEL_SRST_ON			(1 << 7)
/* delay and retry count */
#define KIONIX_RESET_RETRY		3 /* at least 1 */
#define KIONIX_POWER_UP_RETRY 	2
#define KIONIX_POWER_UP_TIME_FRAC	50
#define KIONIX_START_UP_TIME	50
/*****************************************************************************/

/*****************************************************************************/
/* for calibration settings */
/*****************************************************************************/
#define ACCEL_COUNT_1G_2G_RES_12BIT (1 << 10)
#define ACCEL_COUNT_1G_4G_RES_12BIT (1 <<  9)
#define ACCEL_COUNT_1G_8G_RES_12BIT (1 <<  8)
#define ACCEL_COUNT_1G_2G_RES_8BIT  (1 <<  6)
#define ACCEL_COUNT_1G_4G_RES_8BIT  (1 <<  5)
#define ACCEL_COUNT_1G_8G_RES_8BIT  (1 <<  4)

#define CALI_OFF	0
#define CALI_1_FACE	1
#define CALI_6_FACE	6
/*****************************************************************************/
...
static int kionix_accel_clear_interrupt(struct kionix_accel_driver *acceld, u8 int_rel)
{
	int err = 0, loop = KIONIX_I2C_RETRY_COUNT;

	while(loop) {
		err = i2c_smbus_read_byte_data(acceld->client, int_rel);
		if(err < 0){
			loop--;
...
static void kionix_accel_grp2_report_accel_data(struct kionix_accel_driver *acceld)
{
	...
	if(acceld->accel_drdy == 1)
		err = kionix_accel_clear_interrupt(acceld, ACCEL_GRP2_INT_REL);
}
...
static void kionix_accel_grp4_report_accel_data(struct kionix_accel_driver *acceld)
{
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				y = ((s16) le16_to_cpu(accel_data.accel_data_s16[acceld->axis_map_y])) >> acceld->shift;
				z = ((s16) le16_to_cpu(accel_data.accel_data_s16[acceld->axis_map_z])) >> acceld->shift;
				if(acceld->negate_x) x = -x;
				if(acceld->negate_y) y = -y;
				if(acceld->negate_z) z = -z;

				switch(acceld->accel_calisel) {
				case CALI_6_FACE:
					{
						int const * const cx = acceld->accel_cali6[acceld->axis_map_x];
						int const * const cy = acceld->accel_cali6[acceld->axis_map_y];
						int const * const cz = acceld->accel_cali6[acceld->axis_map_z];
						/* calibrated x = ( (x - offset) * 1024
						 * 					- |y| * cx[+y/-y]
						 * 					- |z| * cx[+z/-z] ) / sensitivity
						 *
						 * i.e. ((x - offset) - y effect - z effect) / sensitivity */
						acceld->accel_data[acceld->axis_map_x] =
							( ((x - cx[0]) << 10)
							  - (y >= 0 ? y * cx[2] : -y * cx[3])
							  - (z >= 0 ? z * cx[4] : -z * cx[5]) ) / cx[1];
						acceld->accel_data[acceld->axis_map_y] =
							( ((y - cy[0]) << 10)
							  - (x >= 0 ? x * cy[2] : -x * cy[3])
							  - (z >= 0 ? z * cy[4] : -z * cy[5]) ) / cy[1];
						acceld->accel_data[acceld->axis_map_z] =
							( ((z - cz[0]) << 10)
							  - (x >= 0 ? x * cz[2] : -x * cz[3])
							  - (y >= 0 ? y * cz[4] : -y * cz[5]) ) / cz[1];
					}
					break;
				case CALI_1_FACE:
					acceld->accel_data[acceld->axis_map_x] = x + acceld->accel_cali[acceld->axis_map_x];
					acceld->accel_data[acceld->axis_map_y] = y + acceld->accel_cali[acceld->axis_map_y];
					acceld->accel_data[acceld->axis_map_z] = z + acceld->accel_cali[acceld->axis_map_z];
					break;
				case CALI_OFF:
				default:
					acceld->accel_data[acceld->axis_map_x] = x;
					acceld->accel_data[acceld->axis_map_y] = y;
					acceld->accel_data[acceld->axis_map_z] = z;
					break;
				}
	...
	/* Clear the interrupt if using drdy */
	if(acceld->accel_drdy == 1)
		err = kionix_accel_clear_interrupt(acceld, ACCEL_GRP4_INT_REL);
}
...
static void init_count_1g(struct kionix_accel_driver *acceld)
{
	switch(acceld->accel_pdata.accel_res) {
		case KIONIX_ACCEL_RES_6BIT:
		case KIONIX_ACCEL_RES_8BIT: /* RES 8 */
			switch(acceld->accel_pdata.accel_g_range) {
				case KIONIX_ACCEL_G_8G:
				case KIONIX_ACCEL_G_6G: /* 8G */
					acceld->count_1g = ACCEL_COUNT_1G_8G_RES_8BIT;
					break;
				case KIONIX_ACCEL_G_4G:
					acceld->count_1g = ACCEL_COUNT_1G_4G_RES_8BIT;
					break;
				case KIONIX_ACCEL_G_2G:
				default:
					acceld->count_1g = ACCEL_COUNT_1G_2G_RES_8BIT;
					break;
			}
			break;
		case KIONIX_ACCEL_RES_12BIT: /* RES 12 */
		default:
			switch(acceld->accel_pdata.accel_g_range) {
				case KIONIX_ACCEL_G_8G:
				case KIONIX_ACCEL_G_6G: /* 8G */
					acceld->count_1g = ACCEL_COUNT_1G_8G_RES_12BIT;
					break;
				case KIONIX_ACCEL_G_4G:
					acceld->count_1g = ACCEL_COUNT_1G_4G_RES_12BIT;
					break;
				case KIONIX_ACCEL_G_2G:
				default:
					acceld->count_1g = ACCEL_COUNT_1G_2G_RES_12BIT;
					break;
			}
			break;
	}
...
/* Initialize cali6 content to ideal sensitivity,
 * offset and cross-axis effect should already zeroed */
static void init_cali6(struct kionix_accel_driver *acceld)
{
	int i;
	for(i = 0; i < 3; ++i)
		acceld->accel_cali6[i][1] = acceld->count_1g; /* sensitivity counts/G  */
}
...
/* Returns the 6-face calibration value of the device */
static ssize_t kionix_accel_get_cali6(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
	const int field_count = 6;
	int cx[field_count], cy[field_count], cz[field_count], i;

	read_lock(&acceld->rwlock_accel_data);

	for(i = 0; i < field_count; ++i) {
		cx[i] = acceld->accel_cali6[acceld->axis_map_x][i];
		cy[i] = acceld->accel_cali6[acceld->axis_map_y][i];
		cz[i] = acceld->accel_cali6[acceld->axis_map_z][i];
	}
	...
	return sprintf(buf,
			...
			cz[0], cz[1], cz[2], cz[3], cz[4], cz[5]);
}

/* Allow users to change the 6-face calibration value of the device */
static ssize_t kionix_accel_set_cali6(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
	struct input_dev *input_dev = acceld->input_dev;
	const int field_count = 6;
	const int cali_count = 3 * field_count; /* How many calibration that we expect to get from the string */
	char **buf2;
	long calibration[cali_count];
	int err = 0, i = 0;
	int cx[field_count], cy[field_count], cz[field_count];
	...
	if(kionix_strtok(buf, count, buf2, cali_count) < 0) {
		...
		/* update 6-face calibration */
		if(calibration[1] != 0 &&
		   calibration[field_count + 1] != 0 &&
		   calibration[field_count * 2 + 1] != 0) {
			write_lock(&acceld->rwlock_accel_data);

			for(i = 0 ; i < field_count; i++) {
				acceld->accel_cali6[acceld->axis_map_x][i] = (int)calibration[i];
				acceld->accel_cali6[acceld->axis_map_y][i] = (int)calibration[field_count + i];
				acceld->accel_cali6[acceld->axis_map_z][i] = (int)calibration[field_count * 2 + i];
			}
			write_unlock(&acceld->rwlock_accel_data);
		} else {
			KMSGERR(&acceld->client->dev, \
					"%s: sensitivity values must not be zero. " \
					"sensitivities read: %d, %d, %d.\n", __func__,
					calibration[1], calibration[field_count + 1],
					calibration[field_count * 2 + 1]);
		}
	}
...
/* Returns the calibration method */
static ssize_t kionix_accel_get_calisel(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", acceld->accel_calisel);
}

/* Allow users to select calibration method */
static ssize_t kionix_accel_set_calisel(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
	struct input_dev *input_dev = acceld->input_dev;
	char *buf2;
	const int calisel_count = 1;
	unsigned long calisel;
	int err = 0;

	/* Lock the device to prevent races with open/close (and itself) */
	mutex_lock(&input_dev->mutex);

	if(kionix_strtok(buf, count, &buf2, calisel_count) < 0) {
		KMSGERR(&acceld->client->dev, \
				"%s: No calisel data being read. " \
				"No calisel data will be updated.\n", __func__);
	}
		...
		while(*buf2 == '-')
			buf2++;
		#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35))
		err = kstrtouint((const char *)buf2, 10, (unsigned int *)&calisel);
		if (err < 0) {
			KMSGERR(&acceld->client->dev, \
					"%s: kstrtouint returned err = %d\n", __func__, err);
			goto exit;
		...
		#else
		err = strict_strtoul((const char *)buf2, 10, &calisel);
		if (err < 0) {
			KMSGERR(&acceld->client->dev, \
					"%s: strict_strtoul returned err = %d\n", __func__, err);
			goto exit;
		...
		switch(calisel){
			...
			case CALI_1_FACE:
			case CALI_6_FACE:
				acceld->accel_calisel = (int) calisel;
				break;
			default:
				KMSGERR(&acceld->client->dev, "%s: invalid calisel = %d\n", __func__, (unsigned int) calisel);
				break;
		}
	}
...
static DEVICE_ATTR(calisel, S_IRUGO|S_IWUSR, kionix_accel_get_calisel, kionix_accel_set_calisel);
static DEVICE_ATTR(cali6, S_IRUGO|S_IWUSR, kionix_accel_get_cali6, kionix_accel_set_cali6);
...
static int __devinit kionix_retry_reset(struct kionix_accel_driver *acceld)
{
	int err, i;
	int const power_up_retry = KIONIX_POWER_UP_RETRY;
	u8 ctrl_reg1 = 0; /* common default value */
	u8 srst_reg;

	KMSGINF(&acceld->client->dev, "%s%c\n", __func__, '+');

	/* set CTRL_REG1 to default value 0x0, thus PC1 to 0 */
	err = i2c_smbus_write_byte_data(acceld->client, ACCEL_CTRL_REG1, ctrl_reg1);
	if (err < 0)
		return err;

	/* start software reset */
	srst_reg = i2c_smbus_read_byte_data(acceld->client, ACCEL_SRST_CTRL_REG);
	err = i2c_smbus_write_byte_data(acceld->client, ACCEL_SRST_CTRL_REG, srst_reg | ACCEL_SRST_ON);
	if (err < 0)
		return err;

	/* wait for complete software reset */
	for (i = 0; i < power_up_retry; /* increment below */) {
		++i; /* increment each period slept */
		msleep(KIONIX_POWER_UP_TIME_FRAC);
		srst_reg = i2c_smbus_read_byte_data(acceld->client, ACCEL_SRST_CTRL_REG);
		if (srst_reg & ACCEL_SRST_ON) {
			KMSGINF(&acceld->client->dev, "%s: mslept %d ms, srst still on\n", __func__, KIONIX_POWER_UP_TIME_FRAC * i);
		} else {
			KMSGINF(&acceld->client->dev, "%s: mslept %d ms, srst done\n", __func__, KIONIX_POWER_UP_TIME_FRAC * i);
			break;
		}
	}

	if (i < power_up_retry) {
		/* srst routine finished, restore PC1 */
		ctrl_reg1 = i2c_smbus_read_byte_data(acceld->client, ACCEL_CTRL_REG1);
		KMSGINF(&acceld->client->dev, "%s: crel_reg1=0x%02x, srst_reg=0x%02x. Turning on PC1\n", __func__, ctrl_reg1, srst_reg);
		err = i2c_smbus_write_byte_data(acceld->client, ACCEL_CTRL_REG1, ctrl_reg1 | ACCEL_PC1_ON);
		if (err < 0)
			return err;
	} else {
		/* srst timeout, report fail */
		KMSGERR(&acceld->client->dev, "%s: fails with software reset SRST timed out after %d ms\n", __func__, KIONIX_POWER_UP_TIME_FRAC * i);
		return -ETIMEDOUT;
	}
...
static int __devinit kionix_verify(struct kionix_accel_driver *acceld)
{
	int retval = -1, err;
	int i = 0, j = 0;
	int const reset_retry = KIONIX_RESET_RETRY; /* at least 1 */

	for(i = 0; i < reset_retry; ++i) {
		retval = i2c_smbus_read_byte_data(acceld->client, ACCEL_WHO_AM_I);
		switch (retval) {
			...
			case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
				...
				break;
			default:
				KMSGINF(&acceld->client->dev, "this accelerometer is unknow (0x%02x).\n", retval);
				/* retry reset */
				err = kionix_retry_reset(acceld);
				if (err < 0) {
					KMSGERR(&acceld->client->dev, "%s: kionix_retry_reset fails with error: 0x%02x\n", __func__, err);
					return err;
				}
				continue;
		}
		break; /* retry only at default case, which has continue already */
	}
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (acceld->accel_drdy) {
		...
		err = kionix_accel_clear_interrupt(acceld, ACCEL_INT_REL);
...
static int kionix_accel_resume(struct device *dev)
{
	...
	while(acceld->enable_times > 0) {
		kionix_accel_enable(acceld);
		--acceld->enable_times;
	}

	/* clear interrupt to prevent stuck accel interrupt pin
	 * caused by the lake of proper unregister accel handle */
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(acceld->accel_drdy == 1) {
			err = kionix_accel_clear_interrupt(acceld, ACCEL_INT_REL);
			KMSGINF(&acceld->client->dev, "%s%c %s\n", __func__, ':', "INT_REL cleared for resume");
		}
	}
...
static int kionix_accel_suspend(struct device *dev)
{
	...
	while(atomic_read(&acceld->accel_enabled) > 0){
...
static const struct dev_pm_ops kionix_accel_pmops = {
    SET_SYSTEM_SLEEP_PM_OPS(kionix_accel_suspend, kionix_accel_resume)
};
#define KIONIX_ACCEL_PMOPS (&kionix_accel_pmops)

#else
#define KIONIX_ACCEL_PMOPS NULL
#endif /* CONFIG_PM */
