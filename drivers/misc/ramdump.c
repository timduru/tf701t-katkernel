...
#define DATA_LOGS		"/sdcard/Logs"
#define DATA_LOGS_RAMDUMP	"/sdcard/Logs/ramdump"
#define DATA_MEDIA_RAMDUMP	"/sdcard/Logs/ramdump"
#define DATA_LOGS_LAST_KMSG	"/sdcard/Logs/last_kmsg"
#define MOUNT_POINT	"/dev/block/platform/sdhci-tegra.3/by-name/UDA"
#define MOUNT_DELAY	2000//ms
...
static void last_kmsg_work_function(struct work_struct *dat)
{
	...
	while(1)
	{
		fp_mounts = filp_open(MOUNT_POINT, O_RDONLY, 0);
		if (PTR_ERR(fp_mounts) == -ENOENT)
		{
			printk(KERN_INFO "last_kmsg_work_function:open %s fail!\n", MOUNT_POINT);
			msleep(1000);
		}
		else
		{
