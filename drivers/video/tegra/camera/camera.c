...
int tegra_camera_mclk_on_off(int on)
{
	...
	pr_info("%s(%d)++\n", __func__, on);
	if (!g_camera) {
		pr_info("%s(%d): g_camera == NULL\n", __func__, on);
		return -1;
	}

	mutex_lock(&g_camera->tegra_camera_lock);
	if (on) {
		/* read initial clock info */
		tegra_camera_init_clk(g_camera, clock_init);

		/* enable camera HW clock */
		ret = tegra_camera_enable_clk(g_camera);
		if (ret)
			pr_info("tegra_camera_enable_clk error: %d\n", ret);
	} else {
		/* disable HW clock */
		ret = tegra_camera_disable_clk(g_camera);
		if (ret)
			pr_info("tegra_camera_disable_clk error: %d\n", ret);
	}
	mutex_unlock(&g_camera->tegra_camera_lock);
