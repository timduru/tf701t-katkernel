...
#define TCH_WAKE_LOCK_TIMEOUT		(4*HZ)
...
static int rm_tch_spi_read(u8 u8addr, u8 *rxbuf, size_t len)
{
	...
	if(g_stTs.u8SPILocked)
    {
		...
		if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
			rm_printk("## SPI Read Locked!! 0x%x:%d \n",u8addr,len);
        return OK;
    }
...
static int rm_tch_spi_write(u8 *txbuf, size_t len)
{
    ...
    {
		if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
			rm_printk("## SPI write Locked!! 0x%x:0x%x \n",txbuf[0],txbuf[1]);
        return OK;
    }
    ...
    if(txbuf[0] == 0x11 && (txbuf[1] & 0x1) && u8Bank==0 )
    {
        rm_tch_spi_read(0x38,&u8Data,1);
        if(u8Data != 0x29)
        {
              if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
                rm_printk("## SPI write Locked!! 0x%x:0x%x && 0x38=0x%x\n",txbuf[0],txbuf[1],u8Data);
            return OK;
        }
    }
	mutex_lock(&g_stTs.mutex_spi_rw);
...
static int rm_tch_st_read_image_data(unsigned char *p)
{
       int ret;
       g_pu8BurstReadBuf = p;
       ret = rm_tch_cmd_process(1, g_stRmReadImgCmd, NULL);
       //ret = rm_tch_cmd_process(0, g_stRmTestModeCmd, NULL);
       return ret;
}
...
static u32 rm_tch_ctrl_configure(void)
{
	...
	switch (g_stTs.u8ScanModeState) {
		...
		case RM_SCAN_PRE_IDLE_MODE:
			...
			g_stTs.u8PreScanModeState = RM_SCAN_IDLE_MODE;
		...
		switch (pCmdTbl[_CMD]) {
				...
				if (pCmdTbl[_SUB_CMD] == KRL_SUB_CMD_SET_3V3_REGULATOR) {
					if (ts) {
						if (pCmdTbl[_DATA]) {
							...
							gpio_direction_output(pdata->gpio_3v3, 0);
							ret = OK;
						}
					} else {
						ret = FAIL;
			...
			case KRL_CMD_CONFIG_1V8:/*Need to check qpio setting*/
				...
				if (pCmdTbl[_SUB_CMD] == KRL_SUB_CMD_SET_1V8_REGULATOR) {
					if (ts) {
						if (pCmdTbl[_DATA]) {
							gpio_direction_output(pdata->gpio_1v8, 1);
							ret = OK;
						} else {
							gpio_direction_output(pdata->gpio_1v8, 0);
									ret = OK;
						}
					} else {
						ret = FAIL;
...
#if ENABLE_IEC_TEST                    
                    g_pu8BurstReadBuf[0] = g_stCtrl.bNsSel;
#endif
                }
...
static u32 rm_tch_get_spi_lock_status(u8 *p)
{
	...
	if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
        rm_printk("## SPI_LOCK= %d \n",g_stTs.u8SPILocked);

	u32Ret = copy_to_user(p, &g_stTs.u8SPILocked, sizeof(g_stTs.u8SPILocked));
	if (u32Ret != 0)
		return 0;
	return 1;
}
...
/*=============================================================================*/
//
void t007_dump_debug_buf(unsigned char *buf,int iCount)
{
       int i,j;
       char str[256];
	...
	{
	       //if ((i % (g_stCtrl.bChannelNumberX-1))==0)
	       if ((i % (g_stCtrl.bChannelNumberX+1))==0)
             {
		       sprintf(&str[0],"[%04d]:",i);
		       j=7;//sizeof ("[xxxx]:")
		}
             sprintf(&str[j]," %03d",buf[i]);
             j+=4;//size of (" xx")

             //if ((i % (g_stCtrl.bChannelNumberX-1))==(g_stCtrl.bChannelNumberX-2))
             if ((i % (g_stCtrl.bChannelNumberX+1))==(g_stCtrl.bChannelNumberX))
             {
		       str[j] = 0;
		       printk("%s\n",str);
		       j = 0;
...
static void rm_work_handler(struct work_struct *work)
{
	...
	if (u32Flag & RM_NEED_TO_READ_RAW_DATA) {
      ...
      //if(g_stTs.u8PreScanModeState == RM_SCAN_IDLE_MODE)
      if (((g_stCtrl.bDebugSwitch & SHOW_ST_RAW) == SHOW_ST_RAW)&&(g_stTs.u8PreScanModeState==RM_SCAN_IDLE_MODE)){
             jj=g_stCtrl.bChannelNumberX+g_stCtrl.bChannelNumberY+2;
             g_stTs.u8PreScanModeState = g_stTs.u8ScanModeState;
                    if (pKernelBuffer)
                    iRet = rm_tch_st_read_image_data((u8 *) pKernelBuffer);
                    if(iRet){
                           t007_dump_debug_buf(pKernelBuffer, jj);
                    }
      } else {
		if (pKernelBuffer) {
			...
			if (iRet) {
				...
				if(((g_stCtrl.bDebugSwitch & SHOW_MT_RAW))== SHOW_MT_RAW &&(g_stTs.u8PreScanModeState==RM_SCAN_IDLE_MODE)){
                                 g_stTs.u8PreScanModeState = g_stTs.u8ScanModeState;
                                 t007_dump_debug_buf(pKernelBuffer, g_stCtrl.u16DataLength);
                          }
                    }
             }
       }
...
//nick_hsiao:implementation of ATD tool function
static ssize_t rm_tch_touch_status_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	u32 i;
	struct rm_tch_ts *ts = dev_get_drvdata(dev);
	//emulate command "echo 4 > /sys/bus/spi/devices/spi0.0/self_test"
	unsigned char spi_cmd[1] = {'4'};
	rm_tch_self_test_handler(ts, spi_cmd,  2);

	//nick_hsiao:porting from raydium_selftest

	for( i =0;i<RM_SELF_TEST_READ_COUNT;i++)
	{
		msleep(500);
		if(g_stTs.u8SelfTestStatus == RM_SELF_TEST_STATUS_FINISH)
		{
...
static DEVICE_ATTR(self_test, 0664, rm_tch_self_test_show,
...
//nick_hsiao:implementation of ATD tool function
static DEVICE_ATTR(touch_status, 0640, rm_tch_touch_status_show,
					NULL);
...
static ssize_t raydium_touch_switch_name(struct switch_dev *sdev, char *buf)
{     return sprintf(buf, "Raydium-%4.4x-%4.4x\n",
              g_stTs.u8Version, g_stTs.u8TestVersion);
}
...
void rm_tch_set_variable(unsigned int index, unsigned int arg)
{
	switch (index) {
		...
		case RM_VARIABLE_REPEAT:
			g_stCtrl.bNsRpt = (u8) arg;
			if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
				rm_printk("g_stCtrl.bNsRpt %d\n", g_stCtrl.bNsRpt);
			break;
		case RM_VARIABLE_DPW:
        	mutex_lock(&g_stTs.mutex_ns_mode);
			memcpy(&g_stCtrl.bDP[0],((u8*)arg),3);
			memcpy(&g_stCtrl.bSWCPW[0],(((u8*)arg)+3),3);
        	mutex_unlock(&g_stTs.mutex_ns_mode);
			if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
				rm_printk("g_stCtrl.DPW 0x%x:0x%x:0x%x # 0x%x:0x%x:0x%x\n", g_stCtrl.bDP[0],g_stCtrl.bDP[1],g_stCtrl.bDP[2],
    			                                                    g_stCtrl.bSWCPW[0],g_stCtrl.bSWCPW[1],g_stCtrl.bSWCPW[2]);
			break;
		case RM_VARIABLE_NS_MODE:
        	mutex_lock(&g_stTs.mutex_ns_mode);
			g_stCtrl.bNsMode = (u8)arg;
        	mutex_unlock(&g_stTs.mutex_ns_mode);
			if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
...
static int rm_tch_suspend(struct device *dev)
{
	...
	if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
        rm_printk("## SPI_LOCKED  by suspend!! \n");

	pdata = dev->platform_data;

	// call suspend pinmux
	pdata->suspend_pinmux();
	return 0;
}
...
static int rm_tch_resume(struct device *dev)
{
	...
	if (wake_lock_active(&g_stTs.Wakelock_Initialization))
		wake_unlock(&g_stTs.Wakelock_Initialization);
       rm_printk("Rm_wakelock_start \n");
	wake_lock_timeout(&g_stTs.Wakelock_Initialization,
	        TCH_WAKE_LOCK_TIMEOUT);
	rm_printk("Rm_wakelock_end \n");

       g_stTs.u8SPILocked = 1;
       if(g_stCtrl.bDebugMessage & DEBUG_DRIVER)
             rm_printk("## SPI_LOCKED by resume!! \n");

       rm_ctrl_start(ts);
       rm_printk("Rm_resume_end \n");
       return 0;
}
...
static void rm_tch_spi_shutdown(struct spi_device *spi)
{
	...
	struct rm_spi_ts_platform_data *pdata;
	pdata = g_input_dev->dev.parent->platform_data;
	//refer to "Power Sequence" of RM31081I6 Data sheet
	gpio_direction_output(pdata->gpio_3v3, 0);
...
static int __devexit rm_tch_spi_remove(struct spi_device *spi)
{
	...
	mutex_destroy(&g_stTs.mutex_ns_mode);
...
static int __devinit rm_tch_spi_probe(struct spi_device *spi)
{
	...
	ts->touch_sdev.print_name = raydium_touch_switch_name;
	if(switch_dev_register(&ts->touch_sdev) < 0){
		rm_printk("switch_dev_register for dock failed!\n");
		//goto exit;
	}
