...
int rm_tch_ctrl_scan_start(void)
{
	...
	g_stCtrl.bNsSel = g_stCtrl.bSWCPW[u8NsSel];

	if(u8NsSel < g_stCtrl.bNsMode)
	{
		u8NsSel++;
	}
	else
	...
	if(u8Rpt != g_stCtrl.bNsRpt)
	{
		u8Rpt = g_stCtrl.bNsRpt;
		rm_set_repeat_times(u8Rpt);
	}


	
	rm_tch_spi_byte_write(0x20, g_stCtrl.bDP[u8NsSel]);
	rm_tch_spi_byte_write(0x22, g_stCtrl.bDP[u8NsSel]+2);
	rm_tch_spi_byte_write(0x24, g_stCtrl.bSWCPW[u8NsSel]);
...
#define PARA_ALGORITHM_LEN 128
    ...
    g_stCtrl.bDebugSwitch = pPara[207];
